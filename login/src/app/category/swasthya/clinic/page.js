...
export default function OrganizationAdminClinicListPage() {
  ...
  const loadClinics = useCallback(async () => {
    try {
      setLoading(true);
      setError("");
      const response = await getOrganizationHospitals({ sub_group: "Clinic" });
      setClinics(Array.isArray(response?.hospitals) ? response.hospitals : []);
    } catch (err) {
      setError("Unable to load the clinic list.");
      setClinics([]);
    } finally {
      setLoading(false);
    }
  ...
  const toggleActionMenu = (clinicId, event) => {
  ...
  const handleEditClinic = async (clinic) => {
    ...
    const identifierCandidates = [
      ...
      clinic.company_id ? String(clinic.company_id) : null,
    ].filter(Boolean);
    ...
    const initialValues = {
      ...
      company_id: companyInfoPayload?.company_id || clinic.company_id || "",
      organization_id:
        companyInfoPayload?.organization_id || clinic.company_id || "",
      company_name: companyInfoPayload?.company_name || clinic.company_name || "",
      location:
      ...
      admin_name: companyInfoPayload?.admin_name || clinic.admin_name || "",
      username:
        companyInfoPayload?.username ||
        companyUsername ||
        clinic.username ||
        "",
      email: companyInfoPayload?.email || clinic.email || "",
  ...
  const handleDeleteClinic = async (clinic) => {
    ...
    setActionMenuOpenId(null);
    setActionMenuPosition(null);
    const identifier = clinic.company_id || clinic.id;
    if (!identifier) {
      setActionMessageTone("error");
      setActionMessage("Unable to determine the selected company.");
      return;
    ...
    setDeletingCompanyId(clinic.id);
    try {
  ...
  const renderTable = () => {
    if (loading) {
      return <p className="text-sm text-slate-500">Loading clinics...</p>;
    }
    if (error) {
      return <p className="text-sm font-medium text-rose-600">{error}</p>;
    }
    if (clinics.length === 0) {
      return <p className="text-sm text-slate-500">No clinics found.</p>;
    }
            ...
            {clinics.map((clinic, index) => (
              ...
              >
                ...
                <td className="px-3 py-3 text-slate-600">
                  {clinic.school_category || clinic.sub_group || "—"}
                </td>
                <td className="px-3 py-3 font-medium text-slate-900">
                  {clinic.location || "—"}
                </td>
                ...
                <td className="px-3 py-3 text-slate-600">
                  {clinic.state || clinic.head_office_state || "—"}
                </td>
                <td className="px-3 py-3">
                  ...
                  >
                    {clinic.is_approved ? "Yes" : "Pending"}
                  </span>
                </td>
                <td className="px-3 py-3 text-right text-slate-600">
                  ...
                  >
                    <button
                      type="button"
                      onClick={(event) => toggleActionMenu(clinic.id, event)}
                    ...
                    {actionMenuOpenId === clinic.id && actionMenuPosition && (
                      <div

...
const baseModules = [
  {
    ...
    href: "/category/swasthya/hospital",
    icon: Hospital,
...
export default function Vidya() {
  ...
  const [isCompanyUserSession, setIsCompanyUserSession] = useState(false);
  ...
  const [isCompanyAccountCreateDialogOpen, setCompanyAccountCreateDialogOpen] =
    useState(false);
  const [isCreatingCompanyAccount, setIsCreatingCompanyAccount] =
    useState(false);
  const [companyAccountCreateError, setCompanyAccountCreateError] =
    useState("");
  ...
  const [companyAccountCreateForm, setCompanyAccountCreateForm] = useState({
    organization_id: "",
    company_name: "",
    admin_name: "",
    username: "",
  ...
  const [selectedOrganizationProfileId, setSelectedOrganizationProfileId] =
    useState("");
  const selectedManagementContext = "hospital";
  ...
  useEffect(() => {
    ...
    setCompanyInfoForm((prev) => ({
      ...prev,
      currency_code: matchedCountry.currency || prev.currency_code || "",
      date_format: matchedCountry.date_format || prev.date_format || "",
      language: matchedCountry.language || prev.language || "",
    }));
  ...
  const requireCompletedCompanyProfile = () => {
    const hasLoadedCompanyProfile =
      Boolean(companyInfoForm?.id) ||
      Boolean(companyInfoForm?.organization_id) ||
      Boolean(companyId);

    if (!hasLoadedCompanyProfile) {
      return true;
    }
    ...
    setCompanyInfoError(
      "Complete the organization profile before opening any app.",
    );
    ...
    const hasMultipleOrganizations = organizationProfileOptions.length > 1;
    if (hasMultipleOrganizations && !companyInfoForm?.id) {
  ...
  useEffect(() => {
    ...
    const loadCompanyInfo = async () => {
      ...
      try {
        ...
        setIsCompanyUserSession(
          normalizedUserType === "company_user" ||
            normalizedUserType === "company_admin",
        );
        const userTypeLabel = normalizedUserType.startsWith("organization")
          ? "Organization user"
          : normalizedUserType.startsWith("company")
            ? "Company user"
            : "";
  ...
  const resetCompanyAccountCreateForm = () => {
    setCompanyAccountCreateForm({
  ...
  const handleCompanyAccountCreateFormChange = (event) => {
    const { name, value } = event.target;
    setCompanyAccountCreateForm((prev) => ({
      ...prev,
      [name]: value,
    }));
  };
  ...
  const handleCompanyAccountCreateSubmit = async (event) => {
    event.preventDefault();
    setCompanyAccountCreateError("");

    const payload = {
      organization_id: companyAccountCreateForm.organization_id,
      company_name: companyAccountCreateForm.company_name.trim(),
      admin_name: companyAccountCreateForm.admin_name.trim(),
      username: companyAccountCreateForm.username.trim(),
      email: companyAccountCreateForm.email.trim(),
      phone_number: companyAccountCreateForm.phone_number.trim(),
      password: companyAccountCreateForm.password,
      address: companyAccountCreateForm.address.trim(),
      city: companyAccountCreateForm.city.trim(),
      state: companyAccountCreateForm.state.trim(),
      country: companyAccountCreateForm.country.trim(),
      pin: companyAccountCreateForm.pin.trim(),
      account: true,
      billing: true,
      hrm: false,
      asset: false,
    ...
    if (!payload.organization_id) {
      setCompanyAccountCreateError("Organization is required.");
      return;
    }
    if (!payload.company_name) {
      setCompanyAccountCreateError("Company name is required.");
      return;
    }
    if (!payload.admin_name) {
      setCompanyAccountCreateError("Admin name is required.");
      return;
    }
    if (!payload.username) {
      setCompanyAccountCreateError("Username is required.");
      return;
    }
    if (!payload.email) {
      setCompanyAccountCreateError("Email is required.");
      return;
    }
    if (!payload.phone_number) {
      setCompanyAccountCreateError("Phone number is required.");
      return;
    }
    if (!payload.password) {
      setCompanyAccountCreateError("Password is required.");
      return;
    }
    ...
    try {
      ...
      setCompanyAccountCreateDialogOpen(false);
      resetCompanyAccountCreateForm();
      ...
      setCompanyAccountCreateError(String(detail));
    } finally {
  ...
  const handleCompanyInfoSubmit = async (event) => {
    ...
    try {
      ...
      const normalizedCompanyInfo = {
        ...
        organization_id:
          refreshedCompanyInfo?.organization_id ||
          refreshedCompanyInfo?.company_id ||
          companyInfoForm.organization_id,
        organization_logo_id:
          refreshedCompanyInfo?.organization_logo_id ||
          refreshedCompanyInfo?.company_logo_id ||
          null,
        organization_name:
          refreshedCompanyInfo?.organization_name ||
          refreshedCompanyInfo?.company_name ||
  ...
  const handleSchoolManagementClick = () => {
    window.location.replace("/school-management");
  };

  const resolveHospitalDashboardPath = ({ isSuperuser, userDetails }) => {
    const normalizedUser = userDetails?.user || {};
    const userType = userDetails?.user_type || normalizedUser?.user_type || "";
    const role = (
      userDetails?.role ||
      normalizedUser?.role ||
      ""
    ).toLowerCase();
    const roles = Array.isArray(normalizedUser?.roles)
      ? normalizedUser.roles.map((item) => String(item).toLowerCase())
      : [];

    const isAdminLikeUser =
      normalizedUser?.is_admin === true ||
      normalizedUser?.is_owner === true ||
      role === "admin" ||
      role === "owner" ||
    ...
    if (isSuperuser || isAdminLikeUser) {
      return "/hospital-management/admin_dashboard";
    }
    if (role === "administration" || roles.includes("administration")) {
      return "/hospital-management/administration_dashboard";
    }
    if (role === "doctor" || roles.includes("doctor")) {
      return "/hospital-management/doctor_dashboard";
    }
    return "/login";
  };

  const handleHospitalManagementClick = async () => {
    const username = localStorage.getItem("username");
    const isSuperuser = localStorage.getItem("is_superuser") === "true";
    ...
    if (!requireCompletedCompanyProfile()) {
      return;
    }
    ...
    try {
      ...
      const normalizedUser = userDetails?.user || {};
      const normalizedRoles = Array.isArray(normalizedUser?.roles)
        ? normalizedUser.roles.map((item) => String(item).toLowerCase())
        : [];
      const isAdministrationRole =
        normalizedRoles.includes("administration") ||
        (userDetails?.role || normalizedUser?.role || "").toLowerCase() ===
          "administration";
      ...
      window.location.replace(
        resolveHospitalDashboardPath({ isSuperuser, userDetails }),
      );
    } catch (error) {
      console.error("Failed to load hospital account details:", error);
  ...
  const handleCompanyUserOnlyAccess = (moduleTitle, action) => {
    if (!isCompanyUserSession) {
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-10">
        ...
        <section className="grid justify-items-center gap-8 md:grid-cols-2">
          {availableModules.map((module) => {
            ...
            if (module.enabled && module.href === "/school-management") {
              return (
                <button
                  key={module.title}
                  type="button"
                  ...
                  onClick={() =>
                    handleCompanyUserOnlyAccess(
                      module.title,
                      handleSchoolManagementClick,
                    )
                  }
                >
                  {cardContent}
            ...
            if (module.enabled && module.href === "/category/swasthya/hospital") {
              return (
                <button
                  key={module.title}
                  type="button"
                  className={`${cardClassName} text-center`}
                  onClick={() => window.location.replace(module.href)}
                >
                  {cardContent}
                </button>
              );
            ...
            if (module.enabled && module.href === "/category/swasthya/clinic") {
              return (
                <button
                  key={module.title}
                  type="button"
            ...
            if (module.enabled && module.href === "/payroll-management") {
              return (
                <button
                  ...
                  className={`${cardClassName} text-center`}
                  onClick={() =>
                    handleCompanyUserOnlyAccess(module.title, () =>
            ...
            if (module.enabled) {
              return (
                <button
                  ...
                  onClick={() =>
                    handleCompanyUserOnlyAccess(module.title, () =>
                      window.location.replace(module.href),
                    )
                  }
                >
                  {cardContent}
      ...
      <Dialog
        ...
        onOpenChange={(open) =>
          setWorkspaceAccessDialog((prev) => ({ ...prev, open }))
        }
      >
        <DialogContent className="max-w-md border border-blue-100 bg-white/95">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-gray-600">
              Only Company users are allowed to use this application.
              {workspaceAccessDialog.moduleTitle
                ? ` ${workspaceAccessDialog.moduleTitle} can only be opened by users linked to a company account.`
                : ""}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end">
            <button
              type="button"
              onClick={() =>
                setWorkspaceAccessDialog({ open: false, moduleTitle: "" })
              }
      ...
      <Dialog
        open={isCompanyAccountCreateDialogOpen}
        onOpenChange={(open) => {
          setCompanyAccountCreateDialogOpen(open);
          if (!open) {
            setCompanyAccountCreateError("");
          }
        }}
      >
        <DialogContent className="max-w-3xl border border-blue-100 bg-white/95">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-gray-600">
              Create a company in the legacy company account system and create its admin user at the same time.
            </DialogDescription>
          </DialogHeader>

          <form onSubmit={handleCompanyAccountCreateSubmit} className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700 sm:col-span-2">
                <span>Organization</span>
                <select
                  name="organization_id"
                  value={companyAccountCreateForm.organization_id}
                  onChange={handleCompanyAccountCreateFormChange}
                  required
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="company_name"
                  value={companyAccountCreateForm.company_name}
                  onChange={handleCompanyAccountCreateFormChange}
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="admin_name"
                  value={companyAccountCreateForm.admin_name}
                  onChange={handleCompanyAccountCreateFormChange}
                  placeholder="Dr. Alex Morgan"
                  required
                  className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
              ...
              <div className="rounded-xl border border-dashed border-blue-200 bg-blue-50/70 px-4 py-3 text-sm text-blue-900 sm:col-span-2">
                Company ID will be generated automatically from the company name.
              </div>
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="username"
                  value={companyAccountCreateForm.username}
                  onChange={handleCompanyAccountCreateFormChange}
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="email"
                  name="email"
                  value={companyAccountCreateForm.email}
                  onChange={handleCompanyAccountCreateFormChange}
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="phone_number"
                  value={companyAccountCreateForm.phone_number}
                  onChange={handleCompanyAccountCreateFormChange}
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="password"
                  name="password"
                  value={companyAccountCreateForm.password}
                  onChange={handleCompanyAccountCreateFormChange}
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="address"
                  value={companyAccountCreateForm.address}
                  onChange={handleCompanyAccountCreateFormChange}
                  placeholder="MG Road"
                  className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                />
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="city"
                  value={companyAccountCreateForm.city}
                  onChange={handleCompanyAccountCreateFormChange}
                  placeholder="Pune"
                  className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                />
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="state"
                  value={companyAccountCreateForm.state}
                  onChange={handleCompanyAccountCreateFormChange}
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="country"
                  value={companyAccountCreateForm.country}
                  onChange={handleCompanyAccountCreateFormChange}
                  placeholder="India"
                  className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                />
              ...
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                ...
                <input
                  type="text"
                  name="pin"
                  value={companyAccountCreateForm.pin}
                  onChange={handleCompanyAccountCreateFormChange}
                  placeholder="411001"
                  className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                />
            ...
            {companyAccountCreateError && (
              <p className="text-sm text-red-500">{companyAccountCreateError}</p>
            )}
            ...
            <div className="flex flex-col items-center justify-end gap-3 sm:flex-row">
              ...
              <button
                type="submit"
                disabled={isCreatingCompanyAccount}
              ...
              >
                {isCreatingCompanyAccount ? "Creating..." : "Create company"}
              </button>
            </div>
          </form>
        </DialogContent>
      ...
      >
        <DialogContent
          ...
          className="h-[90vh] !w-[80vw] !max-w-[80vw] overflow-y-auto border border-blue-100 bg-white/95"
        >
          <DialogHeader>
            <DialogTitle className="text-2xl text-gray-900">
            ...
            <DialogDescription className="text-sm text-gray-600">
              Complete the organization details before continuing to any app.
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleCompanyInfoSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              {activeSection === "hospital" && (
                <div className="space-y-6 rounded-2xl border border-blue-100 bg-white p-4">
      ...
      <ImageCropDialog
        key={imageCropDialog.sourceUrl || "image-crop-swasthya"}

...
export default function DoctorTypeList() {
  ...
  const [editingDoctorTypeId, setEditingDoctorTypeId] = useState(null);
  ...
  const [doctorTypeSearch, setDoctorTypeSearch] = useState("");
  ...
  const fetchDoctorTypes = useCallback(async (company_id) => {
    setIsLoading(true);
    try {
      const data = await getAllDoctorType(company_id);
      setDoctorTypes(Array.isArray(data) ? data : []);
      setError(null);
    } catch (err) {
      setError("Unable to load doctor types right now. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (resolvedCompanyId) {
      fetchDoctorTypes(resolvedCompanyId);
    }
  }, [resolvedCompanyId, fetchDoctorTypes]);
  ...
  const startEditing = (doctorType) => {
    setEditingDoctorTypeId(doctorType.id);
    setEditingForm({
      name: doctorType.name || "",
      isApprove: Boolean(doctorType.is_approve),
    });
    setRowError(null);
  };

  const cancelEditing = () => {
    setEditingDoctorTypeId(null);
    setEditingForm({ name: "", isApprove: false });
    setRowError(null);
  };

  const handleSaveDoctorType = async (doctorTypeId) => {
    if (!editingForm.name.trim()) {
      setRowError("Doctor type name cannot be empty.");
      return;
    }
    ...
    setRowLoadingId(doctorTypeId);
    try {
      await updateDoctorType(
        doctorTypeId,
        {
      ...
      await fetchDoctorTypes(resolvedCompanyId);
      cancelEditing();
    } catch (updateError) {
      const apiError =
        updateError.response?.data?.errors?.name?.[0] ||
        updateError.response?.data?.error ||
        "Failed to update doctor type.";
  ...
  const handleDeleteDoctorType = async (doctorTypeId) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this doctor type?",
    );
    ...
    setRowLoadingId(doctorTypeId);
    try {
      await deleteDoctorType(doctorTypeId, resolvedCompanyId);
      setDoctorTypes((prev) =>
        prev.filter((doctorType) => doctorType.id !== doctorTypeId),
      );
      if (editingDoctorTypeId === doctorTypeId) {
        cancelEditing();
      }
      setRowError(null);
    } catch (deleteError) {
      const apiError =
        ...
        "Failed to delete doctor type. Please try again.";
      setRowError(apiError);
    } finally {
      setRowLoadingId(null);
    }
  ...
  const handleCreateDoctorType = async (event) => {
    event.preventDefault();
    if (!formState.name.trim()) {
      setFormError("Doctor type name is required.");
      return;
    }
    ...
    try {
      ...
      const apiError =
        ...
        "Could not create doctor type. Please try again.";
  ...
  const filteredDoctorTypes = doctorTypes.filter((doctorType) => {
    const query = doctorTypeSearch.trim().toLowerCase();
    if (!query) return true;
    return String(doctorType.name || "")
      .toLowerCase()
      .includes(query);
  });
  const totalItems = filteredDoctorTypes.length;
  const totalPages = Math.max(1, Math.ceil(totalItems / itemsPerPage));
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const pagedDoctorTypes = filteredDoctorTypes.slice(startIndex, endIndex);
  ...
  return (
    <div className="p-6 min-h-screen bg-gradient-to-br from-[#f9fafc] to-[#eef3f9]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          {/* <p className="text-sm text-gray-500">
            Manage and create doctor types directly from this view.
          </p> */}
        </div>
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
          ...
          <div className="w-full sm:w-64">
            <input
              type="text"
              value={doctorTypeSearch}
              onChange={(event) => {
                setDoctorTypeSearch(event.target.value);
                setCurrentPage(1);
              }}
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100">
        ...
        ) : filteredDoctorTypes.length === 0 ? (
          <div className="p-10 text-center text-gray-500 text-sm">
            No doctor types available yet. Create the first one to get started.
          </div>
        ) : (
          <div className="overflow-x-auto overflow-y-visible">
            <table className="min-w-full table-fixed divide-y divide-gray-100">
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {pagedDoctorTypes.map((doctorType, index) => (
                  <tr
                    key={doctorType.id}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                      {editingDoctorTypeId === doctorType.id ? (
                        <input
                          type="text"
                        ...
                        doctorType.name || "Unnamed Doctor Type"
                      )}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {editingDoctorTypeId === doctorType.id ? (
                        ...
                        renderStatus(doctorType.is_approve)
                      )}
                    </td>
                    <td className="px-6 py-4 text-sm text-center text-gray-500">
                      {editingDoctorTypeId === doctorType.id ? (
                        <div className="flex items-center justify-end gap-3">
                          <button
                            type="button"
                            disabled={rowLoadingId === doctorType.id}
                            onClick={() => handleSaveDoctorType(doctorType.id)}
                        ...
                        <ActionMenu
                          buttonLabel="Doctor type actions"
                          items={[
                            {
                              label: "Edit",
                              onClick: () => startEditing(doctorType),
                            },
                            {
                              label: "Delist",
                              variant: "danger",
                              disabled: rowLoadingId === doctorType.id,
                              onClick: () => handleDeleteDoctorType(doctorType.id),
                            },
                          ]}
                        />
                      )}
      ...
      {!isLoading && filteredDoctorTypes.length > 0 && (
          ...
          <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Provide the doctor type details to add it to the hospital.
                </p>
              </div>
              <button
                type="button"
            ...
            <form
              onSubmit={handleCreateDoctorType}
              className="space-y-6 px-6 py-6"
            >
              <div className="space-y-2">
                <label
                  htmlFor="doctor-type-name"
              ...
              <label className="flex items-start gap-3 rounded-2xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm text-gray-600">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Approved doctor types appear instantly on the list.
                  </p>
                </div>
              </label>
              ...
              <div className="flex items-center justify-end gap-3 border-t border-gray-100 pt-4">
                ...
                >
                  {isSubmitting ? "Saving..." : "Create Doctor Type"}
                </button>
              </div>
            </form>
          </div>

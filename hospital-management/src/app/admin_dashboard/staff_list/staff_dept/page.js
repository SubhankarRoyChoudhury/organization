...
export default function StaffDepartment() {
  ...
  const fetchStaffDepartments = useCallback(async (company_id) => {
    ...
    try {
      ...
      setError("Unable to load staff departments right now. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  }, [resolvedCompanyId, fetchStaffDepartments]);

  const closeDialog = () => {
    if (isSubmitting) return;
    setIsDialogOpen(false);
    setFormError(null);
    setFormState({ name: "", code: "", isApprove: true });
  };
  ...
  const cancelEditing = () => {
    ...
    setEditingForm({ name: "", code: "", isApprove: false });
    setRowError(null);
  };
  ...
  const handleSaveEdit = async (departmentId) => {
    ...
    if (!editingForm.code.trim()) {
      setRowError("Department code cannot be empty.");
      return;
    }
    if (!resolvedCompanyId) {
    ...
    try {
      ...
      await fetchStaffDepartments(resolvedCompanyId);
      cancelEditing();
    } catch (updateError) {
      const apiError =
        updateError.response?.data?.errors?.name?.[0] ||
        updateError.response?.data?.errors?.code?.[0] ||
        updateError.response?.data?.error ||
        "Failed to update staff department.";
  ...
  const handleDeleteDepartment = async (departmentId) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this staff department?",
    );
    ...
    setRowLoadingId(departmentId);
    try {
      await deleteStaffDepartment(departmentId, resolvedCompanyId);
      ...
      const apiError =
        ...
        "Failed to delete staff department. Please try again.";
      setRowError(apiError);
    } finally {
      setRowLoadingId(null);
    }
  ...
  const handleCreateDepartment = async (event) => {
    ...
    try {
      ...
      const apiError =
        ...
        creationError.response?.data?.errors?.code?.[0] ||
        creationError.response?.data?.error?.[0] ||
        "Could not create staff department. Please try again.";
  ...
  const filteredDepartments = staffDepartments.filter((department) => {
    const query = departmentSearch.trim().toLowerCase();
    if (!query) return true;
    return [department.name, department.code].some((value) =>
      String(value || "")
        .toLowerCase()
        .includes(query),
    );
    ...
    <div className="p-6 pb-10 min-h-screen bg-gradient-to-br from-[#f9fafc] to-[#eef3f9]">
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100">
        {isLoading ? (
          ...
          <div className="p-10 text-center text-gray-500 text-sm">
            No staff departments available yet. Create the first one to get
            started.
          </div>
        ) : (
                      ...
                      {editingDepartmentId === department.id ? (
                        <div className="flex items-center justify-end gap-3">
                          <button
                            ...
                            aria-label="Save staff department"
                          >
                            <Check size={16} />
                          </button>
                          <button
                        ...
                        <ActionMenu
                          buttonLabel="Staff department actions"
                          items={[
                            {
                              label: "Edit",
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Provide the staff department details to add it to the
                  hospital.
                </p>
              </div>
              <button
            ...
            >
              ...
              <div className="space-y-2">
                <label
                  htmlFor="staff-department-code"
                  className="text-sm font-medium text-gray-700"
                >
                  Department Code
                </label>
                <input
                  id="staff-department-code"
                  name="code"
                  type="text"
                  placeholder="e.g. NUR"
              ...
              <label className="flex items-start gap-3 rounded-2xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm text-gray-600">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Approved staff departments appear instantly on the list.
                  </p>
                </div>
              </label>
              ...
              <div className="flex items-center justify-end gap-3 border-t border-gray-100 pt-4">
                ...
                >
                  {isSubmitting ? "Saving..." : "Create Staff Department"}
                </button>
              </div>
            </form>
          </div>
